...
{
    public class GetCrawlJobDataBehaviour : EnalyzerProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(EnalyzerConstants.KeyName.ApiKey, nameof(EnalyzerCrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task EnalyzerCrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.Enalyzer.Core.EnalyzerCrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
